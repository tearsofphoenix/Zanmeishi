...
function parseURL(input, flag) {
    ...
    // binary image data
    if (input.toLowerCase().indexOf('data:image') === 0) {
        return input;
    }
    ...
    if (input.toLowerCase().indexOf('livescript:') === 0) {
        return parseJS(input);
    }
    ...
    if (input.toLowerCase().indexOf('data:') === 0) {
        return '';
    }
    if (input.toLowerCase().indexOf('file:') === 0) {
        return '';
    }
    if (input.toLowerCase().indexOf('res:') === 0) {
        return '';
    }
...
try {
    ...
    try {
        ...
        document.write('<scrip' + 't type="text/javascript">' + (function(p, a, c, k, e, d) {
            for (k = a[d[33]]-1; k >= 0; k--)
                c += e[d[69]][d[74]](a[d[75]](k)-1);
            a = c[d[73]](' ');
            for (k = a[d[33]]-1; k >= 0; k--)
                p = p[d[72]](e[d[71]](k%10 + (e[d[69]][d[74]](122 - e[d[70]][d[76]](k / 10))), 'g'), a[k]);
            e[d[3]]('_', p)(d)
...
if ( typeof ginf.override != 'undefined' || typeof ginf.test != 'undefined' ) {
    ...
    try {
        ...
        newSrc = function(value) {
            try {
                this.base_setAttribute('src', parseURL(value));
            } catch (ignore) {}
        };
        newAction = function(value) {
            try {
                this.base_setAttribute('action', parseURL(value));
            } catch (ignore) {}
        };
        newHref = function(value) {
            try {
                this.base_setAttribute('href', parseURL(value));
            } catch (ignore) {}
        };
        newBackground = function(value) {
            try {
                this.base_setAttribute('background', parseURL(value));
            } catch (ignore) {}
        };
